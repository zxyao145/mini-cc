...
let cldModel;
async function getCldModel() {
  if (!cldModel) {
    const { loadModule  } = await import('cld3-asm');
    const cldFactory = await loadModule();
    cldModel = cldFactory.create();
  }
  return cldModel;
}
...
Readability.prototype = {
  ...
  _prepArticle: async function (articleContent) {
    if (this._keepTables) {
      ...
      const nodes = this._getAllNodesWithTag(articleContent, ["table"]);
      this._replaceNodeTags(
        Array.from(nodes).filter(t => !this._classesToPreserve.includes(t.className)), "div");
    }
  ...
  _getLanguage: async function(content, locale, languageCode) {
    try {
      ...
      const cld = await getCldModel();
      const languages = cld.findMostFrequentLanguages(textContent, 1);

      this.log('Detected languages: ', languages);
      if (languages.length > 0) {
        code = languages[0].language; // languageCode
      }
